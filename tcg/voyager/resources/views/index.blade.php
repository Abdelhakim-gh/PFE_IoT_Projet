...
@section('content')
    <div class="page-content">
        ...
        <div class="cotn" style="border-radius:3px; padding:7px; background:#fff; margin:0;" >
            <script src="https://cdn.jsdelivr.net/npm/chart.js"></script>
        
            @php
                $machines = \App\Machine::all();
                $machineNames = $machines->pluck('name')->toArray();
                $machineValues = $machines->pluck('value')->toArray();
            @endphp
            ...
            @if(auth()->user()->hasRole('admin'))
                <script>
                    ...
                    new Chart(ctx, {
                        type: 'bar',
                        data: {
                            labels: ['Users', 'Machines'],
                            datasets: [{
                                label: 'Statistique d`utilisations',
                                data: [{{ App\Models\User::count() }}, {{ App\Machine::count() }}],
                                backgroundColor: [
            ...
            <script>
                ...
                // Filter out machines with value 0
                const machinesWithData = machines.filter(machine => machine.value !== 0);
            
                // Create arrays of machine names and values
                const machineNames = machinesWithData.map(machine => machine.name);
                const machineValues = machinesWithData.map(machine => machine.value);
            ...
            <script>
                ...
                const data = @json($machines);

                // Filter out rows with slider value 0
                const dataWithData = data.filter(machine => machine.slider !== 0);

                // Create arrays of machine names and values
                const names = dataWithData.map(machine => machine.name);
                const values = dataWithData.map(machine => machine.slider);
